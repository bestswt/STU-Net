...
class STUNetTrainer(nnUNetTrainerV2):
    ...
    def initialize_network(self):
        self.network = STUNet(self.num_input_channels, self.num_classes, depth=[1, 1, 1, 1, 1, 1],
...
class STUNetTrainer_large(STUNetTrainer):
    def initialize_network(self):
        self.network = STUNet(self.num_input_channels, self.num_classes, depth=[2, 2, 2, 2, 2, 2],
...
class STUNetTrainer_huge(STUNetTrainer):
    def initialize_network(self):
        self.network = STUNet(self.num_input_channels, self.num_classes, depth=[3, 3, 3, 3, 3, 3],
                              dims=[96, 192, 384, 768, 1536, 1536],
                              pool_op_kernel_sizes=self.net_num_pool_op_kernel_sizes,
                              conv_kernel_sizes=self.net_conv_kernel_sizes)
